...
var CLIUtils = class _CLIUtils {
  ...
  static downloadLatestBinary = async (binDownloadUrl, cliDir) => {
    ...
    try {
      const zipFilePath = path11.join(cliDir, `downloaded_file_${process.pid}.zip`);
...
var _InsightsHandler = class {
  ...
  async before() {
    if (isBrowserstackSession(this._browser)) {
      await this._browser.execute((data) => {
        if (typeof window.browserstack_executor === "function") {
          window.browserstack_executor(data);
        }
      }, {
  ...
  async _executeCommand(action, args) {
    ...
    if (this._browser.isMultiremote) {
      ...
      return Promise.all(Object.keys(this._caps).map(async (browserName) => {
        ...
        return await browser.execute((data) => {
          if (typeof window.browserstack_executor === "function") {
            window.browserstack_executor(data);
          }
        }, cmd);
    ...
    return await this._browser.execute((data) => {
      if (typeof window.browserstack_executor === "function") {
        window.browserstack_executor(data);
      }
    }, cmd);
